...
impl Floor {
    ...
    fn remove_liftable_rolls(&mut self) {
        self.paper_rolls.clone().into_iter().for_each(|roll| {
            if roll.can_lift {
                self.data[roll.y as usize][roll.x as usize] = String::from(".");
                self.removed += 1;
            }
        });
        self.paper_rolls = vec![];
    }

    fn remove_until_cannot(&mut self) {
        let mut last_removed = -1;
        loop {
            self.remove_liftable_rolls();
            self.process_data();
            if last_removed == self.removed {
                break;
            }
            last_removed = self.removed;
        }
    }
}
...
fn main() {
    ...
    floor.process_data();
    floor.remove_until_cannot();
    println!("total removed rolls: {}", floor.removed)
}
...
@.@.@@@.@.",
        ...
        floor.remove_liftable_rolls();
        floor.process_data();
        assert_eq!(13, floor.removed);
        assert_eq!(58, floor.paper_rolls.len() as u32);
        floor.remove_liftable_rolls();
        floor.process_data();
        assert_eq!(25, floor.removed);
        assert_eq!(46, floor.paper_rolls.len() as u32);
        floor.remove_liftable_rolls();
        floor.process_data();
        assert_eq!(32, floor.removed);
        assert_eq!(39, floor.paper_rolls.len() as u32);
    }
    #[test]
    fn remove_until_cannot() {
        let mut floor = Floor::new(
            "..@@.@@@@.
@@@.@.@.@@
@@@@@.@.@@
