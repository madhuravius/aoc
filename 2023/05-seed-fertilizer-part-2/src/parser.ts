...
import { MapRangeEntry, Almanac, MapRangeData, Seed } from "./types";

// used to parse seeds
export const generateSeedRangeFromString = (input: string): Seed[] => {
  const seeds: Seed[] = [];
  const delimitedSeedsAndRanges = input
    .split(" ")
    .map((num) => parseInt(num, 10))
    .filter((elem) => !isNaN(elem));
  for (let i = 0; i < delimitedSeedsAndRanges.length; i += 2) {
    seeds.push({
      start_range: delimitedSeedsAndRanges[i],
      end_range: delimitedSeedsAndRanges[i] + delimitedSeedsAndRanges[i + 1],
      original_range_size: delimitedSeedsAndRanges[i + 1],
    });
  }
  return seeds;
};
