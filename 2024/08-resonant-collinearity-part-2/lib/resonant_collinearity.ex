  ...
  def main(args \\ []) do
    ...
    case File.read(file_name) do
      {:ok, body} ->
        count =
          body
          |> ResidentCollinearity.Core.sum_all_antinodes(false)

        edge_of_map_count =
          body
          |> ResidentCollinearity.Core.sum_all_antinodes(true)

        IO.puts("Total antinodes: #{count}")
        IO.puts("Total antinodes to edge of map: #{edge_of_map_count}")
